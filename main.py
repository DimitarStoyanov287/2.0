...
pygame.font.init()
# all the images we are going to use for the game#
...
RED_CAR = scale_image(pygame.image.load("img/red-car.png"), 0.55)
GREEN_CAR = scale_image(pygame.image.load("img/green-car.png"), 0.55)

WIDTH, HEIGHT = TRACK.get_width(), TRACK.get_height()
WIN = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("Racing Game!")

# displaying the images we just have inserted#

MAIN_FONT = pygame.font.SysFont("comics-ans", 44)
...
def move_player(driver1):
    keys = pygame.key.get_pressed()
    moved = False

    if keys[pygame.K_a]:
        driver1.rotate(left=True)
    if keys[pygame.K_d]:
        driver1.rotate(right=True)
    if keys[pygame.K_w]:
        moved = True
        driver1.move_forward()
    if keys[pygame.K_s]:
        moved = True
        driver1.move_backward()

    if not moved:
        driver1.reduce_speed()
...
# this is for the size of the image of the grass that is going to show#
# the Grass is on the first place so that is under the image of the Track#
...
# this is for the speed that the game is going to run#
